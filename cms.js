...
/**
 * Cloudinary transformation configurations for optimized image loading
 */
const CLOUDINARY_TRANSFORMS = {
    blur: 'c_fill,w_50,h_50,q_auto:low,f_auto,blur_300', // Ultra-small blur placeholder
    thumbnail: 'c_fill,w_250,h_250,q_auto,f_auto', // CMS grid thumbnails (smaller than gallery)
...
function createImageCard(image, index) {
    ...
    // Optimize image loading for CMS
    // First 8 images are above the fold and should load eagerly
    ...
    // Use optimized thumbnail for CMS grid (250x250 instead of full size)
    ...
    // Add dimensions to prevent layout shift (250x250 for CMS thumbnails)
    img.width = 250;
    img.height = 250;
    ...
    img.onerror = function() {
        ...
        if (this.src !== image.cloudinary_url) {
            this.src = image.cloudinary_url;
        }
    };
...
function openCaptionEditor(imageId, currentCaption, imageUrl) {
    ...
    if (modal && previewImg && captionInput) {
        ...
        // Use medium-sized image for modal preview (faster than full size)
        previewImg.src = generateCloudinaryUrl(imageUrl, 'medium');
